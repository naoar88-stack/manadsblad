...
export function AssetManagerModal({ activity, onSelect, onClose }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.url) throw new Error(data.error || `Serverfel ${res.status}`);
      setGenResult(data);
    } catch (err) {
  ...
  return (
    ...
    >
      <div
        ...
        className="modal-enter bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-100"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          ...
          <button
            ref={closeRef}
            onClick={onClose}
            aria-label="Stäng Asset Manager"
        ...
        {/* Flikar */}
        <div className="flex gap-2 px-6 pt-4" role="tablist">
          {TABS.map(([id, label]) => (
          ...
          <div className="p-6 grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-[420px] overflow-y-auto">
            {IMAGE_POOL.map((img, i) => (
              <button key={i} onClick={() => { onSelect(img); onClose(); }}
                ...
                <img src={img} alt={`Biblioteksbild ${i+1}`}
