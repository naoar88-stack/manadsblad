...
import { LogOut, User, Image as ImageIcon, QrCode, Building2, AlignLeft,
         Cloud, Wifi, Calendar, BookOpen, Check } from 'lucide-react';
...
export function SettingsView({ settings, setSettings, user, onLogout }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-5">
      ...
      {/* Sparat-feedback */}
      {saved && (
        <div className="flex items-center gap-2 text-sm font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-4 py-2.5 rounded-2xl toast-enter">
          <Check size={15} /> Inställningar sparade
        </div>
      )}
        ...
        <Field label="Logotyp" icon={ImageIcon} hint="Rekommenderat: PNG med transparent bakgrund.">
          {settings.yardLogo ? (
            <div className="flex items-center gap-3">
              <img src={settings.yardLogo} alt="Logotyp" className="h-12 w-auto rounded-xl border border-slate-200 bg-slate-50 object-contain p-1" />
              <button onClick={() => update({ yardLogo: '' })} className="text-xs text-rose-500 hover:text-rose-700 font-semibold">Ta bort</button>
            </div>
          ) : (
      ...
      <Card title="Export & QR" description="Styr sidfot, QR-länk och organisationsinfo i exporten.">
        ...
        <Field label="QR-länk" icon={QrCode} hint="QR-koden placeras i sidfoten.">
          <input
            value={settings.qrLink}
            onChange={e => update({ qrLink: e.target.value })}
            className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-200 transition"
      ...
      <Card title="Konto">
        <div className="flex items-center gap-3 py-1">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{user?.isAnonymous ? 'Anonym session — data försvinner vid utloggning' : 'Inloggad med e-post'}</p>
          </div>
